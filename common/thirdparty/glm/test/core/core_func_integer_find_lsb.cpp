...
// This file has divisions by zero to vector isnan
...
#	ifdef NDEBUG
	...
	int i, m, n;
	static unsigned vector[] = {0,32, 1,0, 2,1, 3,0, 4,2, 5,0, 6,1,  7,0,
	...
	for (i = 0; i < n; i += 2) {
		if (ntz1(vector[i]) != vector[i+1]) error(vector[i], ntz1(vector[i]));}
	...
	for (std::size_t k = 0; k < Count; ++k)
	for (i = 0; i < n; i += 2) {
		if (ntz2(vector[i]) != vector[i+1]) error(vector[i], ntz2(vector[i]));}
	TimestampEnd = std::clock();
	...
	for (std::size_t k = 0; k < Count; ++k)
	for (i = 0; i < n; i += 2) {
		if (ntz3(vector[i]) != vector[i+1]) error(vector[i], ntz3(vector[i]));}
	TimestampEnd = std::clock();
	...
	for (std::size_t k = 0; k < Count; ++k)
	for (i = 0; i < n; i += 2) {
		if (ntz4(vector[i]) != vector[i+1]) error(vector[i], ntz4(vector[i]));}
	TimestampEnd = std::clock();
	...
	for (std::size_t k = 0; k < Count; ++k)
	for (i = 0; i < n; i += 2) {
		if (ntz4a(vector[i]) != vector[i+1]) error(vector[i], ntz4a(vector[i]));}
	TimestampEnd = std::clock();
	...
	for(i = 0; i < n; i += 2)
	{
		...
		if(m > 8)
			m = 8;
		if(ntz5(static_cast<char>(vector[i])) != m)
			error(vector[i], ntz5(static_cast<char>(vector[i])));
	}
	TimestampEnd = std::clock();
	...
	for (std::size_t k = 0; k < Count; ++k)
	for (i = 0; i < n; i += 2) {
		if (ntz6(vector[i]) != vector[i+1]) error(vector[i], ntz6(vector[i]));}
	TimestampEnd = std::clock();
	...
	for (std::size_t k = 0; k < Count; ++k)
	for (i = 0; i < n; i += 2) {
		if (ntz6a(vector[i]) != vector[i+1]) error(vector[i], ntz6a(vector[i]));}
	TimestampEnd = std::clock();
	...
	for (std::size_t k = 0; k < Count; ++k)
	for (i = 0; i < n; i += 2) {
		if (ntz7(vector[i]) != vector[i+1]) error(vector[i], ntz7(vector[i]));}
	TimestampEnd = std::clock();
	...
	for (std::size_t k = 0; k < Count; ++k)
	for (i = 0; i < n; i += 2) {
		if (ntz7_christophe(vector[i]) != vector[i+1]) error(vector[i], ntz7(vector[i]));}
	TimestampEnd = std::clock();
	...
	for (std::size_t k = 0; k < Count; ++k)
	for (i = 0; i < n; i += 2) {
		if (ntz8(vector[i]) != vector[i+1]) error(vector[i], ntz8(vector[i]));}
	TimestampEnd = std::clock();
	...
	for (std::size_t k = 0; k < Count; ++k)
	for (i = 0; i < n; i += 2) {
		if (ntz8a(vector[i]) != vector[i+1]) error(vector[i], ntz8a(vector[i]));}
	TimestampEnd = std::clock();
	...
	for (std::size_t k = 0; k < Count; ++k)
	for (i = 0; i < n; i += 2) {
		if (ntz9(vector[i]) != vector[i+1]) error(vector[i], ntz9(vector[i]));}
	TimestampEnd = std::clock();
	...
	for (std::size_t k = 0; k < Count; ++k)
	for (i = 0; i < n; i += 2) {
		if (ntz10(vector[i]) != vector[i+1]) error(vector[i], ntz10(vector[i]));}
	TimestampEnd = std::clock();

	std::printf("ntz10: %d clocks\n", static_cast<int>(TimestampEnd - TimestampBeg));

	if (errors == 0)
		std::printf("Passed all %d cases.\n", static_cast<int>(sizeof(vector)/8));
